...
def main():
    parser = argparse.ArgumentParser(description='Plot wasserstein mask as a circle plot.')
    ...
    wasserstein_dict: dict[str, xr.DataArray] = pickle.load(open(path_to_wasserstein_object, 'rb'))
    wasserstein_mask: xr.DataArray = wasserstein_dict['wasserstein_mask']
    wasserstein_distances: xr.DataArray = wasserstein_dict['wasserstein_distances']

    tested_edges: int = np.sum(wasserstein_mask.values)
    ...
    plot_wasserstein_ranked_differential_communication_circle_plot(
        ...
        top_n_edges=tested_edges,
        cell_types=mnc_object.source_cell_types,
